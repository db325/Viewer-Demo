...
makeTimeStruct(){
    ...
    if(this.From==undefined||this.Until==undefined){
        ...
        this.From=timeInfoObject.timeExtent[0]
        this.Until=timeInfoObject.timeExtent[1]
        //Get the difference
        let tmpVar= this.timeInHours(this.timeInMinutes(this.timeInSeconds(this.range())))
        //Create new date range
        let newUntil=new Date().getTime()+tmpVar*Hr*1000
        this.Until=newUntil
    ...
    if(this.Stepper.indexOf("Minutes")>-1){
        ...
        let  m= this.minute(this.Interval)
         ...
         return Timestruct={
            ...
            hours:this.timeInHours(this.timeInMinutes(this.timeInSeconds(this.range())))
     }
         ...
         let   h=this.hour(this.Interval)
            ...
            return Timestruct={
                ...
                coverageHours:this.timeInHours(this.timeInMinutes(this.timeInSeconds(this.range())))
        }
...
function startAnimation(timeStruct,src,sTime,xTimeFromNow){
    ...
    //Make sure the request time doesn't exceed the range
        if(sTime.getTime()>timeStruct.until){
            console.log("Time Map is ready ..about to start from the beginning......",timeMap)
            ...
            gifshot.createGIF({
                ...
                ,text:"START: "+new Date(timeStruct.from).toISOString() + "  END:"+new Date(picElemArray[ picElemArray.length-1].time).toISOString(),
                'fontSize': '15px',
                fontColor: '#0d0c35 '
            },function(obj) {
                if(!obj.error) {
...
let returnUpdates= await fetch(u+retUpdates)
let returnUpateResponse =await returnUpdates.json()
        if(returnUpateResponse.timeExtent){
            console.log(returnUpateResponse.timeExtent)
            TI.from=returnUpateResponse.timeExtent[0]
            TI.until=returnUpateResponse.timeExtent[1]
            delete(TI.timeExtent)
        }else{
            TI.from=TI.timeInfo.timeExtent[0]
            TI.from=TI.timeInfo.timeExtent[1]
            delete(TI.timeExtent)
        }
return TI
}
...
function stuff2(timestructure,layer){
    ...
    animationId= setInterval(()=>startAnimation(timestructure,src,begin,stopHere ),1000/0.6)
    src.refresh()
  }